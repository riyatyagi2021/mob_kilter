...
class EditFood extends StatefulWidget {
  ...
  int? price;
  String? fooodId;
  List<dynamic>? foodImagee;

   EditFood({Key? key, this.foodName,this.price, this.fooodId, this.foodImagee}) : super(key: key);
...
@override
  ...
  EditFoodRepository editFoodRepository = EditFoodRepository();
  EditFoodModel editFoodModel = EditFoodModel();
  //int pricee=int.parse(price.toString());  int.parse(price.text)

  editFood() async {
    print("${widget.fooodId.toString()}    foooooodid");
    editFoodModel = await editFoodRepository.editFoodApi("",widget.fooodId.toString(), newFoodName.text, newPrice.text,newQuant.text);


    if(editFoodModel.statusCode==1){
      //  this is for without updating picture
     // editFoodModel = await editFoodRepository.editFoodApi("",widget.fooodId.toString(), newFoodName.text, newPrice.text,newQuant.text);
    ...
    print('${editFoodModel.responseData?.message.toString()} New itemmmmmmmmmmm');
  }
