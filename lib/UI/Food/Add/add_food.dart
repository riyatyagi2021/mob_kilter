...
class _AddFoodState extends State<AddFood> {
  ...
  void selectImages() async {
    ...
    if (selectedImages!.isNotEmpty) {
      ...
      Fluttertoast.showToast(msg: "Can select only 2 images");
    }
  ...
  cameraImage() async {
    try {
      ...
      setState(() {
        this.image[1] = File(image.elementAt(1).path);
        //this.image = fileimage;
        //this.image.add(fileimage) ;
  ...
  galleryImage() async {
    try {
      ...
      final fileimage = File(imageFile.path);
      setState(() {
        this.image[2] = File(image.elementAt(2).path);
        // this.image = fileimage;
        //this.image.add(fileimage) ;
      });
    } on PlatformException catch (e) {
                       ...
                       child: ListView.builder(
                           ...
                           itemBuilder:(BuildContext context, int index){
                             return  Container(
                               height: 80,
                               width: 90,
                               child:  imageFileList?[index] == null
                                   ? ClipRRect(
                                   borderRadius: radius,
                                   child: Image.asset(
                                   ...
                                   : ClipRRect(
                                   borderRadius: radius,
                                   child: Image.file(File(imageFileList![index].path))),            //Image.file(image[1])),
                               decoration: BoxDecoration(
                     ...
                     /*     Container(
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: () {
                            if(foodName.text.isNotEmpty && quant.text.isNotEmpty && price.text.isNotEmpty && imageFileList!.length>0){
                              addFood();
                            }else{
  ...
  //int pricee=int.parse(price.toString());  int.parse(price.text)

  addFood() async {
    addFoodModel = await addFoodRepository.addFoodApi("", foodName.text, price.text, imageFileList!, quant.text);


    if(addFoodModel.statusCode==1){
      Navigator.pop(context);
      print("Succcesss");
    }else{
      Fluttertoast.showToast(msg: "${addFoodModel..responseData?.message.toString()}");
      print("Failure");
    }

    print(
        '${addFoodModel.responseData?.foodItemList?[0].name.toString()} New itemmmmmmmmmmm');
  }
}
